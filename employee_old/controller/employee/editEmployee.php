...
<?php
 ...
 if($_POST["employee_id"] != '')  
  ...
  if ($firstName=="" || $_POST["lastName"] == "" || $gender==""|| $nid=="" || $mobileNo=="" || $email=="" ||  $dob==""  ||  $address=="") 
      ...
      $update_employee=$usr->update_employee($employee_id,$firstName,$lastName,$email,$mobileNo,$address,$gender,$nid,$dob,$photoName);
      if ($update_employee) 
            {
      ...
      else 
      {
          ...
          move_uploaded_file($file_temp, $_SERVER['DOCUMENT_ROOT'].'/ems/photo/'.$uploaded_image);
           // print_r($_POST['lastName']);
            ...
            $update_employee=$usr->update_employee($employee_id,$firstName,$_POST["lastName"],$email,$mobileNo,$address,$gender,$nid,$dob,$uploaded_image);
