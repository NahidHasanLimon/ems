...
$query = "SELECT pro.*,CONCAT(emp1.first_name,' ',emp1.last_name) as approved_by_name, CONCAT(emp2.first_name,' ',emp2.last_name) as created_by_name,CONCAT(emp3.first_name,' ',emp3.last_name) as assigned_to_name,pro_cat.name as project_category_name,cl.name as client_name,p_status.status as project_status_name
...
if ($result) {
	...
	$sub_array[] = '<td><a class="viewProject" href="#" id='.$row['id'].'>'. $row["name"] . '</a></td>';
	$sub_array[] = '<td><span class="label label-info">' . $row["client_name"] . '<span></td>';
	$sub_array[] = '<td>' . $row["assigned_to_name"] . '</td>';
	$sub_array[] = '<td><span class="label label-success">' . ($row['start_date'] ? date('d F, Y', strtotime($row['start_date'])) : '') . '</span></td>';
	$sub_array[] = '<td><span class="label label-success">' . ($row['end_date'] ? date('d F, Y', strtotime($row['end_date'])) : '') . '</span></td>';
	if($row["actual_start_date"]==null){
		$sub_array[] = '<td> 
	<button type="button" class="btn btn-rounded btn-warning startBtnProject"  id="'.$row["id"].'" ><i class="fa fa-paper-plane" aria-hidden="true"></i>Start</button></td>';
	}else{
		$sub_array[] = '<td><span class="label label-success">'.($row['actual_start_date'] ? date('d F, Y', strtotime($row['actual_start_date'])) : '').'</span></td>';
	}
	$sub_array[] = '<td> 
	<select class="btn btn-rounded btn-primary selectProjectStatus" data-current_project_status='.$row['project_status'].' name="selectTask_status"id='.$row['id'].' required="" data-id='.$row['id'].'>
	<option value=""> Select Status </status>
                          <option value="1" '.($row["project_status"]==1?"selected" : "" ).' >Prospect</option>
                          <option value="2" '.($row["project_status"]==2?"selected" : "" ).' >Meeting Set</option>
                          <option value="3" '.($row["project_status"]==3?"selected" : "" ).' >RFQ</option>
                          <option value="4" '.($row["project_status"]==4?"selected" : "" ).' >Pitch Prepared</option>                 
                          <option value="5" '.($row["project_status"]==5?"selected" : "" ).' >Quotation Sent</option>                 
                          <option value="6" '.($row["project_status"]==6?"selected" : "" ).' >Work Order Received</option>                 
                          <option value="7" '.($row["project_status"]==7?"selected" : "" ).' >On Going</option>                 
                          <option value="8" '.($row["project_status"]==8?"selected" : "" ).'>Work Done</option>                 
                          <option value="9" '.($row["project_status"]==9?"selected" : "" ).'>Bill Sent</option>                 
      </select>
	</td>';
	if($row["actual_end_date"]==null && $row["actual_start_date"]!=null && $row['request_for_end']=='0'){
		$sub_array[] = '<td> 
	<button type="button" class="btn btn-rounded btn-warning endBtnProject"  id="'.$row["id"].'" ><i class="fa fa-paper-plane" aria-hidden="true"></i>END</button></td>';
	}else{
		$sub_array[] = '<td><span class="label label-success">'.($row['actual_end_date'] ? date('d F, Y', strtotime($row['actual_end_date'])) : '').'</span></td>';
	}
